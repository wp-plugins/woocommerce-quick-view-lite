...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	?>
	<div class="post-<?php echo $pid; ?> product type-product status-publish has-post-thumbnail shipping-taxable purchasable product-type-grouped instock" id="product-<?php echo $pid; ?>" itemtype="http://schema.org/Product" itemscope="">
	...
	$pagepostsparents1 = $wpdb->get_results( $wpdb->prepare( " SELECT * FROM `".$table_prefix."posts` where post_parent=%d ", $pid ) );
	$sale = '';
	foreach($pagepostsparents1 as $pagepostsparent1)
	{
		$child_ID1 = $pagepostsparent1->ID;
		
		$sale .= get_post_meta( $child_ID1, '_sale_price', true);
	...
	?>	
		...
		<form enctype="multipart/form-data" method="post" class="cart">
			<table class="group_table" cellspacing="0">
				<tbody>
				<?php
					$pagepostsparents = $wpdb->get_results(" SELECT * FROM `".$table_prefix."posts` where post_parent='$pid' ");

					foreach($pagepostsparents as $pagepostsparent)
					{
						$child_ID = $pagepostsparent->ID;
						
						$child_product = new WC_Product( $child_ID );
						
						?>
							<tr>
								<td>
									<div class="qty-r"><input type="number" size="4" class="input-text qty text" title="Qty" value="1" name="quantity[<?php echo $child_ID; ?>]" min="1" step="1"></div>
								</td>

								<td class="label">
									<label for="product-<?php echo $child_ID; ?>">
										<a href="<?php echo get_permalink( $child_ID ); ?>"><?php echo $child_product->post->post_name; ?></a>
									</label>
								</td>
								...
								<td class="price">
									<?php
										...
										echo $child_product->get_price_html()
